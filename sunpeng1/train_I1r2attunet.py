...
from network import I1R2AttUNet
...
@click.command()
@click.option('-e', '--epoch', 'epoch_num', help='Number of training epoch', type=int, default=100, show_default=True)
@click.option('-b', '--batch', 'batch_size', help='Number of batch size', type=int, default=4, show_default=True)
...
def main(epoch_num, batch_size, lr, num_gpu, img_size, data_path, log_path,
    ...
    cp_path = log_path / 'inceptionv1/checkpoint'
    ...
    net = I1R2AttUNet(in_ch=dataset.img_channels, out_ch=dataset.num_classes, base_ch=32)
    ...
    for epoch in range(start_epoch, epoch_num):
        ...
        try:
            ...
            if (epoch + 1) % cp_intvl == 0:
                cp_file = cp_path / f'cp_train_last.pth'
                cp.save(epoch, net.module, optimizer, str(cp_file))
